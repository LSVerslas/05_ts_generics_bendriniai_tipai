...
function getTodos(url: string): Promise<TodoObjIf[]> {
    fetch(url).then((reso) => Response.json()).catch((err) => {
        console.warn('Error fetching', error);
    }
}


// // Assume that the getTodos function fetches data from an API or a database
// // and returns a Promise that resolves to an array of objects representing todos.
...
//             // Resolve the Promise with the todos data
//             resolve(todos);
...
// // Usage: Fetch todos and print them
// getTodos()
//     .then(todos => {
